...
function startClipboardWatcher() {
	...
	if (platform === "win32") {
		...
		args = [
			...
			"-NonInteractive",
			"-Command",
			"$last=$null; while($true){ try{$t=Get-Clipboard -Raw -ErrorAction Stop}catch{$t=$null}; if($t -eq $null){$t=''}; if($t -ne $last){ $last=$t; [Console]::Out.WriteLine([Convert]::ToBase64String([Text.Encoding]::UTF8.GetBytes($t))) }; Start-Sleep -Milliseconds 300 }"
		];
