...
async function renderPage(file, pageNum, scale) {
	const data = new Uint8Array(await promises.readFile(file));
	const page = await (await pdfjs.getDocument({ data }).promise).getPage(pageNum);
	...
	const items = (await page.getTextContent()).items.filter((i) => i && typeof i.str === "string" && i.str.length > 0).map((i) => {
		const t = transform(viewport.transform, i.transform);
		return {
			x: Math.round(t[4]),
			y: Math.round(t[5]),
			fontSize: Math.round(i.height || 10),
			text: i.str
		};
	});
	return {
	...
	ws.register({
		...
		handler: async (req, res) => {
			...
			try {
				...
				if (pathname === "/bilingual-reader/page" && req.method === "GET") return json(res, 200, await renderPage(decodeURIComponent(u.searchParams.get("path") || ""), parseInt(u.searchParams.get("page") || "1", 10) || 1, parseFloat(u.searchParams.get("scale") || "2") || 2));
