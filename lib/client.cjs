...
function makePdfView({ h, useState, useEffect }) {
	return function PdfView(props) {
		...
		useEffect(() => {
			...
			setErr("");
			setData(null);
			fetch(`/bilingual-reader/page?path=${encodeURIComponent(file)}&page=${page}&scale=${scale}`).then((r) => r.json()).then((j) => {
				if (alive) {
		...
		if (err) return h("div", { style: {
			color: "#e53e3e",
			padding: 12
		} }, "页图加载失败：" + err);
		if (!data) return h("div", { style: {
			padding: 12,
			color: "#a0aec0"
		} }, "加载页图…");
		...
		return h("div", { style: {
			...
			disabled: page <= 1
		}, "上一页"), h("span", void 0, "第 " + page + " 页"), h("button", { onClick: () => setPage(page + 1) }, "下一页")), h("div", { style: {
			position: "relative",
			width: data.width,
			maxWidth: "100%",
			overflow: "auto"
