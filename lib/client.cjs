...
function makeTermsView({ h, useState, useEffect }) {
	return function TermsView(props) {
		...
		return h("div", { style: {
			...
			margin: "8px 0 4px",
			color: "var(--dsw-alias-label-primary)"
		} }, "领域图（DAG）"), graph ? h("div", { style: { marginBottom: 12 } }, h("div", { style: termsStyle }, "根：" + (graph.roots ?? []).join(", ")), Object.entries(graph.edges ?? {}).map(([parent, children]) => h("div", {
			key: parent,
			style: termsStyle
		}, parent + " → " + (children ?? []).join(", ")))) : h("p", { style: termsStyle }, "（无领域数据）"), h("h3", { style: {
			fontSize: 14,
			margin: "8px 0 4px",
			color: "var(--dsw-alias-label-primary)"
