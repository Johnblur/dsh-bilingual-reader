...
function makeReader({ h, useState, useEffect, useCallback, useRef }) {
	...
	return function BilingualReader(props) {
		...
		async function doTranslate(copied) {
			...
			} else if (hits.length > 1) setMatchSel({
				kind: "multiple",
				count: hits.length
			});
		...
		return h("div", { style: {
			...
			alignItems: "center",
			fontSize: 12
		} }, h("span", { style: { color: matchSel.kind === "matched" ? "var(--dsw-alias-state-success-primary)" : "var(--dsw-alias-state-warn-primary)" } }, matchSel.kind === "matched" ? "✓" : "⚠"), h("span", { style: { color: "var(--dsw-alias-label-secondary)" } }, matchSel.kind === "matched" ? "已匹配到原文，使用上下文翻译" : matchSel.kind === "multiple" ? "该片段在原文出现 " + (matchSel.count ?? 0) + " 处，未使用上下文（直接翻译）" : "未在原文中定位到该片段，直接翻译")) : void 0, h("div", { style: {
			color: "#666",
			fontSize: 13,
			maxHeight: 130,
			overflow: "auto"
