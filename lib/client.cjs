	...
	return function BilingualReader(props) {
		...
		async function doTranslate(copied) {
			...
			try {
				...
				if (seq === reqSeq.current) {
					setSelResult(res);
					setSelError(false);
					clog("translate ok: len=" + String(res ?? "").length);
				}
			} catch (err) {
			...
			const poll = async () => {
				...
				try {
					...
					if (!r.ok) warn = `宿主路由返回 HTTP ${r.status}（插件路由被 Desktop 的浏览器访问围栏拦下）`;
						...
						} else if (!d.lines && !d.fileOk && (d.polls ?? polls) > 25) warn = `剪贴板助手已启动（读法 ${d.mode || "未知"}）但始终读不到内容，宿主路由被请求 ${d.polls ?? polls} 次仍未收到任何文本` + (d.bytes ? `；stdout 收到 ${d.bytes} 字节但无法解析` : "；stdout 无任何输出");
					}
				} catch (e) {
		...
		return h("div", { style: {
			...
			return h("div", { style: { marginTop: 8 } }, h("button", {
				...
				lineHeight: 1.6
			} }, detected ? h("div", {}, "识别为：" + detected) : void 0, detectedDomain && !domain ? h("div", {}, "LLM 判断领域：" + domainLabel(detectedDomain)) : void 0, matchSel.kind !== "empty" ? h("div", {}, matchSel.kind === "matched" ? "已匹配到原文，使用上下文翻译" : matchSel.kind === "multiple" ? "该片段在原文出现 " + (matchSel.count ?? 0) + " 次，使用第一次出现的上下文" : "未在原文中定位到该片段，直接翻译") : void 0) : void 0);
