...
function makeReader({ h, useState, useEffect, useCallback }) {
	...
	return function BilingualReader(props) {
		...
		return h("div", { style: {
			...
			alignItems: "center",
			flexWrap: "wrap"
		} }, clipAvailable ? h("span", { style: {
			color: "#48bb78",
			fontWeight: 600
		} }, "✓ 已启用自动翻译") : h("button", { onClick: () => void onClipboardTranslate() }, "翻译选中（在 PDF 里复制后）"), h("label", void 0, "上下文"), h("input", {
			type: "range",
			min: 0,
			max: 800,
			step: 50,
			...
			style: { width: 160 }
		}), h("span", void 0, contextLen + " 字")), h("div", { style: { marginTop: 10 } }, sel ? h("div", {}, h("div", { style: {
			color: "#718096",
			fontSize: 13,
			marginBottom: 6,
			maxHeight: 130,
			overflow: "auto"
		} }, "原文：" + sel.selection), h("div", { style: { lineHeight: 1.7 } }, selResult || "翻译中…")) : h("p", { style: {
			color: "#a0aec0",
			marginTop: 4
		} }, "在 PDF 里选中一段文字并复制，即可自动翻译（或点「翻译选中」）。"))));
	};
}
//#endregion
