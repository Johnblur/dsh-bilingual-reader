...
function makeReader({ h, useState, useEffect, useCallback, useRef }) {
	...
	return function BilingualReader(props) {
		...
		const [customDomains, setCustomDomains] = useState(loadLangBlob("dsh-bl.customDomains", []));
		...
		const load = useCallback(async () => {
			...
			if (controller.detectDomain) try {
		...
		useEffect(() => {
			saveLangBlob("dsh-bl.customDomains", customDomains);
		}, [customDomains]);
		...
		async function doTranslate(copied) {
			...
			let context = "";
			if (!selText || !hasDoc) setMatchSel({ kind: "empty" });
			else {
			...
			const effDomain = domainRef.current || docDomainRef.current || void 0;
			try {
		...
		async function onClipboardTranslate() {
			...
			try {
				...
				if (!r.ok) diag = ` [host route HTTP ${r.status}]`;
				else {
			...
			if (!text) {
				setSelResult("（未能读取剪贴板：请先在 PDF 里选中并复制，然后在本页按 Ctrl+V）" + diag);
				setSelError(true);
				return;
			}
			try {
		...
		useEffect(() => {
			...
			const poll = async () => {
				...
				try {
					const r = await fetch("/bilingual-reader/clipboard");
					if (!r.ok) warn = `宿主路由返回 HTTP ${r.status}（插件路由被 Desktop 的浏览器访问围栏拦下）`;
					else {
						const j = await r.json();
						...
						} else if (!d.lines && !d.fileOk && polls > 25) warn = `剪贴板助手已启动（读法 ${d.mode || "未知"}）但始终读不到内容，宿主路由被请求 ${polls} 次仍未收到任何文本` + (d.bytes ? `；stdout 收到 ${d.bytes} 字节但无法解析` : "；stdout 无任何输出");
					}
				} catch (e) {
					warn = `无法访问宿主路由：${e instanceof Error ? e.message : String(e)}`;
				}
				let browserDenied = false;
				if (!available && typeof navigator !== "undefined" && navigator.clipboard?.readText) try {
		...
		const addCustomDomain = () => {
			...
			if (!name) return;
			if (!customDomains.includes(name)) setCustomDomains((prev) => [...prev, name]);
			setAddDomainName("");
		...
		return h("div", { style: {
		...
		}, h("path", { d: "M5 12h14" }), h("path", { d: "M13 6l6 6-6 6" }))), h("select", {
			value: target,
			onChange: (e) => {
			...
			onChange: (e) => {
				const v = e.target.value;
				if (v === "__custom") setShowAddDomain(true);
				else setDomain(v);
			},
			style: selectStyle
			...
			fontSize: 12,
			color: "#555"
		} }, contextLen + " 字")), clipWarn ? h("div", { style: {
			marginTop: 8,
			fontSize: 12,
			lineHeight: 1.6,
			wordBreak: "break-word",
			color: "var(--dsw-alias-state-warn-primary)"
		} }, "⚠ " + clipWarn) : void 0, showAddLang || showAddDomain ? h("div", { style: {
			display: "flex",
			gap: 8,
			alignItems: "center",
			marginTop: 8
		} }, h("input", {
			value: showAddDomain ? addDomainName : addLangName,
			placeholder: showAddDomain ? "领域名（如 经济学）" : "语言名（如 法语 / French）",
			onChange: (e) => {
				if (showAddDomain) setAddDomainName(e.target.value);
				else setAddLangName(e.target.value);
			},
			style: {
				...inputBase,
				flex: 1
			...
			onClick: () => {
				if (showAddDomain) addCustomDomain();
				else addCustomLang();
			},
			className: BTN_CLS
		}, "添加"), h("button", {
			...
			return h("div", { style: { marginTop: 8 } }, h("button", {
			...
			}, h("span", { style: { fontSize: 12 } }, (hasWarn ? "⚠ " : "") + (summary || "查看状态")), h("span", { style: {
				fontSize: 10,
				opacity: .7
				...
				lineHeight: 1.6
			} }, detected ? h("div", {}, "识别为：" + detected) : void 0, manualDomain ? h("div", {}, "手动指定领域：" + domainLabel(domain)) : void 0, detectedDomain ? h("div", {}, "LLM 判断领域：" + domainLabel(detectedDomain) + (manualDomain ? "（手动指定优先，未采用）" : "")) : void 0, loadErr ? h("div", { style: { color: "var(--dsw-alias-state-warn-primary)" } }, loadErr) : void 0, matchSel.kind !== "empty" ? h("div", {}, matchSel.kind === "matched" ? "已匹配到原文，使用上下文翻译" : matchSel.kind === "multiple" ? "该片段在原文出现 " + (matchSel.count ?? 0) + " 次，使用第一次出现的上下文" : "未在原文中定位到该片段，直接翻译") : void 0) : void 0);
		})(), h("div", { style: { marginTop: 10 } }, sel || selResult ? h("div", { style: {
			display: "flex",
			flexDirection: "column",
			gap: 6
		} }, sel ? h("div", { style: {
			...
			maxHeight: 130,
			overflow: "auto"
		} }, "原文：" + sel.selection) : void 0, h("div", { style: {
			display: "flex",
			gap: 8,
		...
		} }, selResult || "翻译中…"), selResult && !selError ? h("button", {
...
async function post(path, body) {
	...
	if (!res.ok) {
		const detail = data && typeof data.error === "string" ? data.error : raw ? raw.slice(0, 300) : "(空响应)";
