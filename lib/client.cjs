...
*/
function fuzzyMatch(doc, sel, threshold = .6) {
	const normDoc = normalizeForMatch(doc);
	const selTokens = tokenize(normalizeForMatch(sel));
	...
	for (let i = 0; i + selCount <= docTokens.length; i++) {
		const score = overlapScore(docTokens.slice(i, i + selCount), selTokens);
	...
	if ((best === null || best.score < threshold) && selCount > 1) for (let d = -1; d <= 1; d++) {
		const w = selCount + d;
		if (w <= 0) continue;
		for (let i = 0; i + w <= docTokens.length; i++) {
			const score = overlapScore(docTokens.slice(i, i + w), selTokens);
			if (best === null || score > best.score) best = {
				start: starts[i],
				score
			};
	...
	return function BilingualReader(props) {
		...
		async function doTranslate(copied) {
			...
			if (!selText) setMatchSel({ kind: "empty" });
				...
				const m = fuzzyMatch(doc.fullText, copied, .6);
				if (m) {
		...
		return h("div", { style: {
			...
			alignItems: "center",
			fontSize: 12
		} }, h("span", { style: { color: matchSel.kind === "not-found" ? "var(--dsw-alias-state-error-primary)" : "var(--dsw-alias-state-success-primary)" } }, matchSel.kind === "not-found" ? "⚠" : "✓"), h("span", { style: { color: "var(--dsw-alias-label-secondary)" } }, matchSel.kind === "matched" ? "已匹配到原文，使用上下文翻译" + (matchSel.score !== void 0 ? "（重合 " + Math.round(matchSel.score * 100) + "%）" : "") : matchSel.kind === "multiple" ? "在原文找到 " + (matchSel.count ?? 0) + " 处相近内容，使用最匹配处上下文" + (matchSel.score !== void 0 ? "（重合 " + Math.round(matchSel.score * 100) + "%）" : "") : "未在原文中定位到该片段，直接翻译")) : void 0, h("div", { style: {
			color: "#666",
			fontSize: 13,
			maxHeight: 130,
			overflow: "auto"
